...
4. Run `npm run prisma:generate` to regenerate the client at `generated/prisma/`.

## Rules
- Always include `id`, `createdAt`, and `updatedAt` on every model.
- Use `@map` and `@@map` to keep DB column/table names in snake_case while model names stay PascalCase.
...
## Domain reminder
Check `specs/CONTEXT.md` — the model name and field names must align with the canonical domain glossary.
